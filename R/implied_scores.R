...
implied_scores <- function(fit) {
    ...
    p <- length(y_names)
    i <- diag(p)
    sigma_yy <- solve(i - beta) %*% (gamma %*% phi %*% t(gamma) + psi) %*% solve(i - t(beta))
    sigma_xx <- phi
    ...
    implied_mean_x <- implied_mean[x_names]
    ...
    for (i in y_names) {
        y_hat_rsquare[i] <- 
            suppressWarnings(cor(dat[, i], y_hat[, i], use = "pairwise.complete.obs")^2)
    }
    ...
    if (check_summary != "") {
        stop(paste0("Some results cannot be reproduced. The implied scores cannot be trusted. \n",
