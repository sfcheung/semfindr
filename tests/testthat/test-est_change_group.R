...
set.seed(3214)
dat0 <- dat[1:75, ]
dat0$gp <- sample(c("Alpha", "Beta", "Gamma"), nrow(dat0), replace = TRUE)
fit0 <- lavaan::sem(mod, dat0, group = "gp")
fit0_15 <- lavaan::sem(mod, dat0[-5, ], group = "gp")

rerun_out <- lavaan_rerun(fit0, parallel = FALSE, skip_all_checks = TRUE)
rerun_15 <- rerun_out$rerun[[5]]
