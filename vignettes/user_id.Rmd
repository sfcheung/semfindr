...
vignette: >
  %\VignetteIndexEntry{Use Case ID}
...
`semfindr::lavaan_rerun` supports user-supplied case ids. This makes
...
The list of reruns now use `id` as the names:

```{r}
head(fit_rerun$rerun[1:3])
```

As shown below most diagnostic functions will use case id
