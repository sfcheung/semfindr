...
This article illustrates how to use `semfindr` to
...
(See `vignette("casewise_scores", package = "semfindr")`).
...
as well as Mahalanobis distance, using the output of
`lavaan_rerun()`:
...
that is extreme is not necessarily the case
...
Strong invariance is still not rejected again, if Case 100 is
removed.
...
generalized Cook's distance (there always
are, due to the way it is computed), there
...
real datasets, please see @aguinis_best-practice_2013.
