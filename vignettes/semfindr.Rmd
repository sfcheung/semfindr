...
the implied scores of outcome variables. See the help
...
`base::saveRDS()` such that we can load it for sensitivity
analysis later, without the need to repeat this step in each
R session.
...
One intuitive way to assess casewise influence is to compute
...
Another intuitive measure of influence is to compute
...
The all-in-one function `influence_stat()` can also be used
...
`influence_stat()` and users use them in exactly the same
