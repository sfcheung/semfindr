---
title: "Selecting Cases In Rerun"
...
Instead of rerunning fitting the model *n* times, once
with each case removed, it is possible to use some criteria
to select cases to be included in the reruns. This is useful
when the speed to fit the model is slow and/or the sample
size is very large.
...
the argument `to_rerun` of `semfindr::lavaan_rerun()`:
...
`to_rerun` should be a vector of case IDs:
...
endogenous variables. The residuals of a case on all
endogenous variables will be computed, and the Mahalanobis
...
Note that selecting cases by this method *can* miss some
influential cases. Unlike multiple regression, this distance
