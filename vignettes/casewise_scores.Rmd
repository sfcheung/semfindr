...
This vignette explains the computational shortcut implemented in `semfindr` to approximate the casewise influence. From my experience, the approximation is most beneficial when the sample size *N* is large, where the approximation works better and the computational cost for refitting models *N* times is high.
...
indicates the partial derivative of the casewise loglikelihod with respect to the parameter `m1~iv1`. Because the sum of the partial derivatives across all observations is zero at the maximum likelihood estimate with the full sample ($\hat \theta_m$; i.e., derivative of loglikelihood of the full data is 0), $- s_i(\theta_m)$ can be used as an estimate of the partial derivative of the loglikelihood at $\hat \theta_m$ **for the sample without observation $i$**. This information can be used to approximate the maximum likelihood estimate for $\theta_m$ when case $i$ is dropped, denoted as $\hat \theta_{m(-i)}$
...
where $\nabla \ell_i(\hat \theta)$ is the gradient vector of the casewise loglikelihood with respect to the parameters (i.e., score). The $N / (N - 1)$ term is used to adjust for the decrease in sample size (not 100% sure whether this is needed, but it is trivial in large samples). This procedure should be the same as equation (4) of [Tanaka et al. (1991)](https://doi.org/10.1080/03610929108830742) (p. 3807) and is related to the one-step approximation described by [Cook and Weisberg (1982)](p. 182).

### Comparison

The approximation is implemented in the `semfindr::est_change_raw_approx()` functon:
...
We can use the approximate parameter change to approximate the gCD (see also Tanaka et al., 1991, equation 13, p. 3811):
...
This is implemented in the `semfindr::est_change_approx()` function:
...
It seems that the approximation would underestimate the actual gCD, although the rank ordering is almost identical. This is discussed also in Tanaka et al. (1991), who also proposed a correction by applying a one-step approximation after the correction (currently not implemented due to the need to recompute scores with updated parameter values).
...
They are pretty close. To approximate the change in chi-squared, as well as other chi-squared-based fit indices, we can use the `semfindr::fit_measures_change_approx()` function:
...
### Comparing exact and approximated changes in fit indices
...
Cook, R. D., & Weisberg, S. (1982). *Residuals and influence in regression.* New York: Chapman and Hall. http://conservancy.umn.edu/handle/11299/37076
