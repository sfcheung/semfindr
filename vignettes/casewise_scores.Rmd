...
This is implemented in the `semfindr::est_change_approx()`:
...
It seems that the approximation would underestimate the actual gCD, although the rank ordering is similar. This is discussed also in Tanaka et al. (1991), who also proposed a correction by applying a one-step approximation after the correction (currently not implemented due to the need to recompute scores with updated parameter values).
...
They are pretty close. To approximate the change in chi-squared, as well as other chi-squared based fit indices, we can use `semfindr::fit_measures_change_approx()` function:
