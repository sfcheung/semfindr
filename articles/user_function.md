# Sensitivity Analysis: Arbitrary User Function

## Goal

This article illustrates how to identify cases that are influential on
an arbitrary statistics computed by any function, using the
[`semfindr`](https://sfcheung.github.io/semfindr/) package
(**cheung_semfindr_2026?**).

Readers are assumed to have a basic understanding on how to use
[`lavaan_rerun()`](https://sfcheung.github.io/semfindr/reference/lavaan_rerun.md),
as illustrated in [this
article](https://sfcheung.github.io/semfindr/articles/semfindr.html).

## Case Influence on Any Statistic

The function
[`user_change_raw()`](https://sfcheung.github.io/semfindr/reference/user_change_raw.md)
can be use to compute case influence measure for any statistics as long
as it is provided a function that:

- accepts a `lavaan` object (e.g., the output of
  [`lavaan::cfa()`](https://rdrr.io/pkg/lavaan/man/cfa.html) or
  [`lavaan::sem()`](https://rdrr.io/pkg/lavaan/man/sem.html)).

- returns a numeric vector of the statistics. If named, the names will
  be used. If not, names will be created automatically.

Different scenarios will be illustrated below.

## Case 1: Reliability

We first illustrate computing case influence on reliability.

The sample dataset `HolzingerSwineford1939` from the `lavaan` package
...
A confirmatory factor analysis model is to be fitted to the items `x1`
to `x9`:
...
We first do LOO analysis, fitting the model *n* times, *n* being the
...
Suppose we are interested in the reliability of the three factors. They
...
fit_rel
#> 
#> Information about each composite is provided below, followed by reliability coefficients.
#> 
#> ----------visual----------
#> 
...
We can compute the influence of each case using
[`user_change_raw()`](https://sfcheung.github.io/semfindr/reference/user_change_raw.md):
...
The argument `user_function` is the argument used to compute the
statistics. NOTE: The output needs to be a numeric vector.

No other arguments are needed in this case because `compRelSEM()`
expects the `lavaan` object as the first argument.
...
Therefore, if the value is positive for a case, the statistic,
reliability in this case, is larger if this case is included.

Conversely, if the value is negative for a case, the statistic is
decreases if this case is included.
...
As shown by both the printout and the plot, including the first case
reduces the reliability of `visual` by 0.021.

We can verify this by fitting the model and compute the reliability
...
Suppose we use the compute case influence on R-squares. This is the
...
The R-squares can be retrieved in different ways. For example, it can be
extracted by
[`lavInspect()`](https://rdrr.io/pkg/lavaan/man/lavInspect.html) from
`lavaan`:
...
This example illustrates an advanced scenario. The function
[`lavInspect()`](https://rdrr.io/pkg/lavaan/man/lavInspect.html)
requires one more argument, `what`, to know that we want the R-squares.
Such arguments require by `user_function` can be passed directly to
[`user_change_raw()`](https://sfcheung.github.io/semfindr/reference/user_change_raw.md),
...
The function
[`user_change_raw()`](https://sfcheung.github.io/semfindr/reference/user_change_raw.md)
can also be used for function for which the `lavaan`-class object is not
the first argument. Please refer to the help page of
[`user_change_raw()`](https://sfcheung.github.io/semfindr/reference/user_change_raw.md)
on how to set this argument.
